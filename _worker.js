...
function createVLESSSub(userID_Path, hostName) {
	...
	userIDArray.forEach((userID) => {
		...
		portArray_https.forEach((port) => {
			...
			const commonUrlPart_https = `:${port}?encryption=none&security=tls&sni=${hostName}&type=ws&host=${hostName}&path=%2F%3Fed%3D2048#${hostName}-HTTPS%26alpn%3Dh2%2Chttp%2F1.1%26fp%3Dchrome&alpn=h2%2Chttp%2F1.1&fp=random`;
